...
class RTS_Accessibility_Toolkit {
    ...
    /**
     * Render settings page (alias for admin integration)
     */
...
html.rts-contrast img,
html.rts-contrast video {
    filter: invert(1) hue-rotate(180deg);
}
html.rts-contrast .rts-a11y-container {
    filter: invert(1) hue-rotate(180deg);
}
...
html.rts-darkmode .rts-a11y-container {
    filter: invert(1) hue-rotate(180deg);
}
...
html.rts-saturate .rts-a11y-pill,
html.rts-saturate .rts-a11y-panel {
    filter: none;
}
...
html.rts-focus main,
html.rts-focus article,
html.rts-focus [role="main"] {
    position: relative;
    z-index: 9999;
}
...
html.rts-calm .rts-a11y-pill,
html.rts-calm .rts-a11y-panel {
    filter: none;
}
...
html.rts-monochrome .rts-a11y-pill,
html.rts-monochrome .rts-a11y-panel {
    filter: none;
}

html.rts-font-boost :is(p, li, span, div, a, button, input, textarea, select, label) {
    font-size: calc(1em + var(--rts-font-add, 0px)) !important;
}

html.rts-weight-boost :is(p, li, h1, h2, h3, h4, h5, h6, span, div, a) {
    font-weight: var(--rts-weight-value, 400) !important;
}

html.rts-lineheight-boost :is(p, li, h1, h2, h3, h4, h5, h6) {
...
html.rts-textalign :is(p, li, h1, h2, h3, h4, h5, h6, div:not(.rts-a11y-pill):not(.rts-a11y-panel):not(.rts-cc-grid):not(.rts-cc-module)) {
    ...
    private function render_markup($is_single) {
        ...
        <div class="rts-a11y-container" id="rts-container">
            ...
            <div id="rts-ruler" aria-hidden="true">
                <button type="button" class="rts-ruler-close" aria-label="Close ruler" title="Close ruler">×</button>
            </div>
...
(function() {
    ...
    function updateFont() {
        ...
        if (fontSize > 0) {
            ...
            elements.container.classList.add('rts-font-boost');
            elements.container.style.setProperty('--rts-font-add', addPx + 'px');
        } else {
            elements.container.classList.remove('rts-font-boost');
    ...
    function updateWeight() {
        ...
        if (fontWeight > 0) {
            elements.container.classList.add('rts-weight-boost');
            elements.container.style.setProperty('--rts-weight-value', weightValues[fontWeight]);
        } else {
            elements.container.classList.remove('rts-weight-boost');
    ...
    function updateLineHeight() {
        ...
        if (lineHeight > 0) {
            elements.container.classList.add('rts-lineheight-boost');
            elements.container.style.setProperty('--rts-lineheight-value', lineHeights[lineHeight]);
        } else {
            elements.container.classList.remove('rts-lineheight-boost');
    ...
    function updateZoom() {
        ...
        if (pageZoom === 100) {
            elements.container.classList.remove('rts-zoom');
            // Reset viewport
            ...
            if (elements.zoomBtn) {
                elements.zoomBtn.setAttribute('data-zoom', '100');
                elements.zoomBtn.setAttribute('aria-label', "<?php esc_attr_e('Page zoom: 100%', 'rts-a11y'); ?>");
            }
        } else {
            elements.container.classList.add('rts-zoom');
            // Use viewport scaling for better compatibility
            const viewport = document.querySelector('meta[name=viewport]');
            const scale = pageZoom / 100;
            const content = `width=device-width, initial-scale=${scale}, maximum-scale=${scale}, user-scalable=yes`;
            ...
            if (elements.zoomBtn) {
                elements.zoomBtn.setAttribute('data-zoom', pageZoom.toString());
                elements.zoomBtn.setAttribute('aria-label', `<?php esc_attr_e('Page zoom:', 'rts-a11y'); ?> ${pageZoom}%`);
            }
        }
        
        if (label) {
            label.textContent = `${pageZoom}%`;
        }
    }
